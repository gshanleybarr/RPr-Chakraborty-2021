...
acs_covid %>% filter(is.na(bpov_pct)) %>% st_drop_geometry() %>%
  ...
  kable_styling() 
#missing_data_table <- filter(acs_covid, is.na(bpov_pct)) %>% st_drop_geometry() %>% t() %>% kable(digits=2, caption = "Example County with Missing Data") %>% kable_styling()
#rownames(missing_data_table) <- c("FIPS Code", "State FP", "County", "County State","COVID Rate", "Dis Percent", "White Percent", "Black Percent", "Native American Percent", "Asian Percent", "Other Percent", " Non-Hispanic White Percent", "Hispanic Percent", "Non-Hispanic Non-White Percent", "BPOV Percent", "APOV Percent", "Percent Ages 5-17", "Percent Ages 18-34", "Percent Ages 35-64", "Percent Ages 65-74", "Percent Over Age 75", "Percent Male", "Percent Female", "Population", "Cases", "Latitude", "Longitude") 
#row.names=c("FIPS Code", "State FP", "County", "County State","COVID Rate", "Dis Percent", "White Percent", "Black Percent", "Native American Percent", "Asian Percent", "Other Percent", " Non-Hispanic White Percent", "Hispanic Percent", "Non-Hispanic Non-White Percent", "BPOV Percent", "APOV Percent", "Percent Ages 5-17", "Percent Ages 18-34", "Percent Ages 35-64", "Percent Ages 65-74", "Percent Over Age 75", "Percent Male", "Percent Female", "Population", "Cases", "Latitude", "Longitude"),
# kbl(col.names = c("County Data", "Data")) %>% 
...
### Maping Percent Disability by Race
```{r map-distribution-of-percent-race}
...
The set of maps above depict percent disability by race. From the upper left corner, the maps depict the spatial distribution of percent disability of White respondents, Black, Native American, Asian, and Other, respectively. The map series assists in identifying clusters of people with disabilities by race. For example, the second map highlights the clustering of Black people with disabilities in the Southeastern US, while there is a high denisty of Native Americans with disabilities in the four corners region of the US. 
...
tmap_save(disability_by_race, here("results", "figures", "disability_by_race.png"))
## Save image of map plot
```
...
Finally, in a third iteration of the reproduction report, we focused on improving data visualizations, particularly regarding the cohesion and cartographic design of maps in the report, the legibility of Table 1, a table representing missing data, and on the narrative flow of the discussion and conclusion of the report. 
