...
author: "Joseph Holler, Junyi Zhou, Peter Kedron, Drew An-Pham, Derrick Burt. Grayson Shanley Barr"
date: "`r Sys.Date()`"
output: pdf_document
editor_options:
  markdown:
...
Version 3.0 \| First Created July 7, 2021 \| Updated October 2nd, 2023
...
#filter(acs_covid, is.na(bpov_pct)) %>% st_drop_geometry() %>% t() %>% kable()  

# county with missing data
acs_covid %>% filter(is.na(bpov_pct)) %>% st_drop_geometry() %>%
  t() %>%
  as.data.frame() %>%
  kable(digits=2, caption = "Example County with Missing Data") %>%
...
tm_spatialepi_clusters \<- tm_shape(state) + tm_fill("gray98") + tm_shape(acs_covid) + tm_polygons(col = "isCluster", palette = "-Oranges", popup.vars = c("fips", "clusterID"), colorNA = NULL, title = "SpatialEpi Kulldorff COVID-19 Clusters", border.col = "white", lwd = 0.2, border.alpha = 0.2) + tm_shape(state) + tm_borders("grey", lwd = 0.5) + tm_layout( legend.position = c("left", "bottom"), legend.title.size = 0.8, legend.text.size = 0.6 )

## Map Percent with Disability by Race

**Unplanned deviation for reproduction**: We also map the spatial distribution of the percent of people with any disability by race to examine geographic patterns associated with the overarching independent variable (percentage of people with disability).  

```{r map-distribution of percent with disability by race}

library(tmap)

# Create a list of variables to map
variables_to_map <- c("white_pct", "black_pct", "native_pct", "asian_pct", "other_pct")

# Define custom legend titles
custom_legend_titles <- c(
  "White Alone",
  "Black Alone",
  "Native Alone",
  "Asian Alone",
...
# Create a list of maps with custom legend titles
maps_list <- lapply(seq_along(variables_to_map), function(i) {
  var <- variables_to_map[i]
  custom_title <- custom_legend_titles[i]
  tm_shape(acs_covid) +
    tm_polygons(var,
      title = paste("Map for Disability Percent:", custom_title),
      style = "fisher",
      border.alpha = 0.2,
      lwd = 0.2,
      palette = "Oranges",
...
disability_by_race <- tmap_arrange(maps_list)
disability_by_race
```
...
tm_spatialepi_hier <-
  ...
  tm_polygons(
    col = "xcluster",
    palette = c("lightgoldenrod1", "tomato", "lightskyblue1"),
    colorNA = NULL,
...
tm_spatialepi_gini <-
  ...
  tm_polygons(
    col = "xcluster",
    palette = c("lightgoldenrod1", "thistle3", "lightskyblue1"),
    colorNA = NULL,
...
# Your existing code for gee_diff

# Calculate the mean and standard deviation
summary_mean <- colMeans(gee_diff) %>% round(digits = 3)
summary_sd <- apply(gee_diff, 2, sd) %>% round(digits = 3)

# Create a data frame for the summary
summary_data <- data.frame(
  Statistic = c("Mean", "Standard Deviation"),
  Value = c(paste(summary_mean, collapse = ", "), paste(summary_sd, collapse = ", "))
)

# Create a Kable table from the summary data
summary_table <- summary_data %>%
  kable("html", col.names = c("Statistic", "Value")) %>%
  kable_styling(full_width = FALSE) %>%
  #add_caption("Summary of difference between reproduction independent variables and original independent variables")

# Print the Kable table
cat(as.character(summary_table))
...
## Reproduction Analysis Findings
...
### Review of Spatial Visual Analysis

For example, the choropleth map in the **first part** of the reproduction analysis (Figure 2) reveals an identical spatial pattern to that of the original study's Figure 1, confirming the equivalence of our dependent variable with that of the original study.
...
This spatial visual analysis reveals many regions in which disability rates are high but COVID-19 incidence was low, including rural New England, Appalachia, and northern Michigan.
Conversely, many hotspots of COVID-19 incidence had low rates of disability, including metropolitan areas in New England, South Florida, and Chicago.
These two spatial distributions help explain the negative bivariate relationship between disability rates and COVID-19 incidence.
...
The original study used Pearson's correlation to test bivariate relationship between variables that are non-normally distributed, and we revised this to use the nonparametric Spearman's correlation coefficient given that the data did not fit a normal distribution.
...
However, biological sex may not be correlated with COVID-19 incidence rates, as the percentage of females with disabilities changes in both direction and significance.

In the reproduction, we additionally added a series of maps that compare percent with disability by race in the US. It is notable that among black Americans, PwD are clustered in the South. Meanwhile, people that identify as Asian and with disability are concentrated on the West coast. In contrast, white Americans with disabibilites are distrubuted fairly evenly across the country. 

### Review of the Kulldorff Spatial Scan Statistic
...
When we extend the interpretation of clusters to use the local relative risk score of any county within a cluster (Figure 5), we find significantly more variance in COVID risk within states, resulting in 139 unique GEE clusters.
...
### Commentary on Original Study Conclusions and Research Design: Sources of Uncertainty
...
#### Modifiable Areal Unit Problem, Variable Measurement, & Spatial Dependency
...
## Critiques of Study
Two potential sources of bias in the results of the study arise from the exclusion of data from Puerto Rico, Hawaii, Alaska, and institutionalized peoples. Although including regions outside of the continental US, could have led to challenges in the configuration of the spatial clustering, including these regions could also have provided more aggregate data on potential associations between PwD and socio-demographic characteristics. In addition, although there is not likely to be accurate data on COVD-19 incidence and institutionalized peoples, this reflects a substantial oversight on the part of the national government to neglect to collect health and demographic information on individuals living in ‘institutionalized group quarters’. The lack of inclusion of institutionalized groups likely underestimate the findings that PwDs are more likely to experience multiple threats that jeopardize their health based on disability, racial/ethnic minority, and poverty status given that COVID-19 incidence is higher among people living in close quarters and people of color are disproportionately represented in the prison population (black Americans are imprisoned at 5 times the rate of white Americans) (https://www.sentencingproject.org/reports/the-color-of-justice-racial-and-ethnic-disparity-in-state-prisons-the-sentencing-project/).

## Unplanned Deviations in the Reproduction Analysis
The reproduction analysis intentionally deviated slightly from the original study in order to conduct  robustness tests on the results of the original analysis and to use an Open Source computational environment (R Studio). In addition to these planned deviations in the reproduction analysis, there were a handful of deviations from the original analysis in the reproduction, including: the calculation of bivariate tests, results comparing SaTScan and SpatialEpi Packages, mapping disability rates, and  missing data errors.
...
### Bivariate Tests
In addition, we planned to test the independent variables for normality prior to using the Pearson's r correlation coefficient for bivariate tests of correlation between the independent variables and COVID-19 incidence rates.
...
### SaTScan vs. SpatialEpi Packages, Conceptualizations of Spatial Clusters, and GEE Models
...
In order to compare our reproduction using the SpatialEpi package to the original study using SaTScan software, we also ran the spatial scan statistics in SaTScan.
...
In the future it would be thought provoking to investigate if a subsequent analysis could be conducted at a finer grain scale such as the census tract level. While conducting the analysis at such a small scale would be unlikely to meaningfully change many of the spatial clusters, cartographic visualization of the relative risk would be able to highlight neighborhoods or particular regions of cities that have been disproportionately impacted by COVID-19. These results would likely support results from the original study that found that PwDs are more likely to experience multiple threats that jeopardize their health based on disability, racial/ethnic minority, and poverty status.

In addition, it would also be meaningful to conduct a reanalysis of the study over a longer temporal span to examine if PwDs are continue to experience multiple threats that jeopardize their health based on disability, racial/ethnic minority, and poverty status even as the pandemic subsided since 2020. An ethnographic study could additionally provide a beneficial complement to the quantitative findings; for example, a human geographer could conduct  interviews with PwD in a number of the documented spatial clusters from the Chakraborty (2021) study to provide insight through qualitative methods. A corresponding ethnography would assist in depicting the story on the ground behind the figures found in the study.

# Conclusion
In conclusion, although some of the results differed from Chakraborty's analysis because of differences in analytical methods, conceptualizations, and computational environments, the reproduction results still suggest that PwDs are more likely to experience multiple threats that jeopardize their health based on the convergence of their disability, racial/ethnic minority, and poverty status.
...
Further work is needed to reliably infer relationships between minority PwDs and COVID-19 morbidity and mortality, especially at finer geographic scales and over longer temporal spans.

# Rationale for an Updated Report
...
Subsequently, in the second version of the reproduction report, we improved our approaches to reproduction studies by revising pre-analysis plans and post-analysis reports into legible code RMarkdown documents.
...
Finally, in a third iteration of the reproduction report, we focused on improving data visualizations, particularly regarding the cohesion and cartographic design of maps in the report, the legibility of Table 2, a table representing missing data, and on the narrative flow of the discussion and conclusion of the report.

## Further Opportunities to Build Upon Results
There are opportunities to further improve data visualizations of study results. I found it challenging to interpret the maps comparing results from the SaTScan and SpatialEpi packages, therefore I focused most of my time on improving the cartographic visualization of these results. An additional means to improve the reproducibility of the study would be for Chakraborty to make the original analysis code accessible to other researchers and students, which would support the ethos of a collective scientific commons for knowledge to be shared and built upon.
